...
Multi-session protocol: how to start, pause, resume, and manage context across days/weeks. The learner's progress lives in `~/ai-systems/`. Two files matter most: `progress.json` (the durable record) and `session-state.md` (the resume pointer).

---

## `session-state.md` schema

A single markdown file at `~/ai-systems/session-state.md`. Overwritten at every checkpoint. Keep it short — under 50 lines. The skill reads this at the start of every Warm Resume session.
...
[1-2 sentences. "Mid-lesson on L3.S2 retrieval fundamentals. Just finished BM25 vs dense; about to start hybrid search." Be specific enough that you (or another harness) could resume cold.]
...
- [Any partial exercise in `exercises/2026-05-07-foo/` that wasn't finished]
...
When `~/ai-systems/session-state.md` exists, this is the standard "user is back" flow.

### Step 1: Read state

Read `progress.json` and `session-state.md`. Note:
- Days since last session
- Mid-lesson or mid-exercise state
- Top of SR queue (overdue items)
- Open threads

### Step 2: Pick a proposal

Priority order:
1. **Mid-lesson/mid-exercise from <14 days ago** → resume that exact thing
2. **SR queue has overdue items** → propose review session, then continue forward
...
| "I have to leave in 10 min" | Run shortened version — quick SR review, no new lesson |
| "where am I?" | Show full curriculum position (read from `progress.json`) |

### Step 5: Execute

Once they confirm, **don't re-explain or preamble**. Begin the lesson/review/exercise immediately.
...
3. Update `progress.json`:
   ...
   - If an exercise was in flight: log it as `in_progress` with the directory path
...
2. **Quick review pass** (optional, ~3 min). Pick 2-3 SR items due in the next 2 days and quiz them. This bakes today's material in.
...
2. If it has meaningful progress (any topic with status `in_progress` or `solid`+):
   > "I have your `progress.json` but no session pointer — looks like the session-state file was deleted or this is a new harness. You're at [layer/topic from progress.json]. Want me to pick up there, or do a quick recalibration first?"
3. If `progress.json` is near-empty (just an init): run **diagnostic-lite** — 3-4 questions across whichever layers had any topic touched — then propose a starting point and write a fresh `session-state.md`.
...
- **Codex / Codex CLI:** start a new task; `~/ai-systems/session-state.md` is the bridge
...
- Anything written to `~/ai-systems/` is portable.
