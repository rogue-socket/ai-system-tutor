...
If your project is a CLI (Option E sometimes), "deploy" means it runs reliably on a fresh machine — `git clone <url>`, `cd <repo>`, `uv sync`, `uv run python ...` (each as a separate command; works in every shell). Document the steps in README.
